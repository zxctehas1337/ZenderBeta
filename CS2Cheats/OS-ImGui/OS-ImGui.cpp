...
namespace OSImGui
{
    ...
    void OSImGui::SwitchButton(const char* str_id, bool* v)
    {
        ...
        // Create invisible button to handle interactions
        ImGui::PushID(str_id);
        ...
        // Background color - green when on, gray when off
        ImVec4 offColor = ImVec4(0.2f, 0.2f, 0.2f, 1.0f);
        ImVec4 onColor = ImVec4(0.1f, 0.6f, 0.1f, 1.0f);
        ImVec4 mixedColor;
        
        mixedColor.x = offColor.x * (1.0f - t) + onColor.x * t;
        mixedColor.y = offColor.y * (1.0f - t) + onColor.y * t;
        mixedColor.z = offColor.z * (1.0f - t) + onColor.z * t;
        mixedColor.w = offColor.w * (1.0f - t) + onColor.w * t;
        
        ImU32 bgColor = ImGui::GetColorU32(mixedColor);
        
        if (hovered)
        {
            ImVec4 hoveredOffColor = ImVec4(0.25f, 0.25f, 0.25f, 1.0f);
            ImVec4 hoveredOnColor = ImVec4(0.15f, 0.7f, 0.15f, 1.0f);
            
            mixedColor.x = hoveredOffColor.x * (1.0f - t) + hoveredOnColor.x * t;
            mixedColor.y = hoveredOffColor.y * (1.0f - t) + hoveredOnColor.y * t;
            mixedColor.z = hoveredOffColor.z * (1.0f - t) + hoveredOnColor.z * t;
            mixedColor.w = hoveredOffColor.w * (1.0f - t) + hoveredOnColor.w * t;
            
            bgColor = ImGui::GetColorU32(mixedColor);
        }
        
        // Rendering background with rounded corners
        ...
        // Circle position with animation
        float circleX = p.x + Radius + t * (Width - Radius * 2);
        float circleY = p.y + Height / 2;
